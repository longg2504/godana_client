...
export default function ImgSlideOnMap({ place }) {
  const { placeList, loading } = UseFetchPlace();
  const theme = useTheme();
  const maxSteps = place
    ? place?.images.length
  ...
  return (
    <Box sx={{ maxWidth: 400, flexGrow: 1, borderRadius: "25px" }}>
      ...
      <MobileStepper
        ...
        nextButton={
          <Button
            ...
            style={{ opacity: buttonOpacity, top: "50px" }}
            onMouseOut={() => setButtonOpacity(1)}
            onMouseLeave={handleMouseLeave}
          >
            {theme.direction === "rtl" ? (
